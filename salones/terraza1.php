...
if ($_SERVER["REQUEST_METHOD"] == "POST" || isset($_GET['action'])) {
    ...
    if ($action === 'viewReservations') {
        ...
        if ($tableId) {
            // Consultar las reservas futuras para esta mesa
            ...
            // Si es una petición AJAX, solo devolvemos el HTML
        ...
        if ($action === 'occupy') {
            ...
            // Verificar si existe un conflicto de horarios con otra reserva
            $sqlCheckConflict = "SELECT * FROM tbl_reservations 
                                 WHERE table_id = :tableId 
                                 AND reservation_date = :reservationDate
                                 AND (
                                     (start_time <= :endTime AND end_time >= :startTime)
                                 )";
            $stmtCheckConflict = $conexion->prepare($sqlCheckConflict);
            $stmtCheckConflict->bindParam(':tableId', $tableId, PDO::PARAM_INT);
            $stmtCheckConflict->bindParam(':reservationDate', $reservationDate);
            $stmtCheckConflict->bindParam(':startTime', $startTime);
            $stmtCheckConflict->bindParam(':endTime', $endTime);
            $stmtCheckConflict->execute();
        
            if ($stmtCheckConflict->rowCount() > 0) {
                // Conflicto de horario detectado
                echo "<script>
                        alert('Error: La mesa ya tiene una reserva en este horario.');
            ...
            // Mantener el estado de la mesa como reservado
...
<body>
    ...
    <div class="container2">
        ...
        <div class="grid">
            ...
            foreach ($tables as $row) {
                ...
                // Si está libre y tiene reservas, mostrar amarilla
                // Si está libre y no tiene reservas, mostrar verde
    ...
    <!-- Añadir este formulario oculto para eliminar reservas -->
